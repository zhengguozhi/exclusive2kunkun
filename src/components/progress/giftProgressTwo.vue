 ...
 * @LastEditTime: 2020-07-29 17:26:25
 -->
<template>
  <div class="gift-progress mc" v-show="bolOnline == 1">
    ...
    <div
      class="btn-gift bsbb flex-center mc"
  ...
  methods: {
    payinit () {
      payinitHttp().then((res = {}) => {
        if (isDev) {
          res.btnList = [{ rewardStr: '', money: '100' }, { rewardStr: '', money: '200' }, { rewardStr: '', money: '200' }, { rewardStr: '', money: '200' }, { rewardStr: '', money: '200' }, { rewardStr: '', money: '200' }, { rewardStr: '', money: '200' }]
          res.currStep = 3
          res.rewardStep = 1
          res.nextMoneyStr = '22'
          res.bolOnline = 1
        ...
        this.canClick = true
        this.colorList = gradientColors('#eb757d', '#eb757d', res.btnList.length + 1)
        if (res.rewardStep > 0) {
